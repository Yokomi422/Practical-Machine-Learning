...
export default function Supple({ children, props }: { children: ReactNode, props: string }) {
    return (
      <Details summary={<summary>{props}</summary>}>
        ...
        そこで、行列を用いて一般化してみましょう。 
        説明変数が2個以上になる場合は、<InlineMath math="x"/>をベクトルとして<InlineMath math="\hat{y}"/>を表現します。
        ここで、<InlineMath math="\boldsymbol{θ}"/>は
        ...
        上では、目的変数yが一つの場合を考えましたが、今回は目的変数yがm個ある場合を考えます。前回と同様に、損失関数は最小二乗誤差(MSE Mean Squared Error)
        を用います。 
        <InlineMath math="\boldsymbol{y}"/>は、
        ...
        となります。これが<InlineMath math="\boldsymbol{0}"/>になればいいので、
        <BlockMath math="-\frac{1}{m} \sum_{i=1}^{m} (y_i - \theta^T x_i) x_i = \boldsymbol{0}" />
        そして、行列の形で表現すると、以下のようになります。
        <BlockMath math="X^T(X\theta - y) = 0" />
        これを直感的には分かりずらいかもしれませんが、<InlineMath math="\sum"/>を分解して、分解した式を行列で表現すると、上の式になります。 
        これを<InlineMath math="\boldsymbol{θ}"/>について解くと、
        <BlockMath math="\boldsymbol{θ} = (\boldsymbol{X}^\mathsf{T} \boldsymbol{X})^{-1} \boldsymbol{X}^\mathsf{T} \boldsymbol{y}"/>
        となります。これが正規方程式です。
        ただし、<InlineMath math="\boldsymbol{X}^\mathsf{T} \boldsymbol{X}"/>が正則である必要があります。
      </Details>
    );
  }